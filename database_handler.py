    ...
    try:
        async with connection_pools["write"].acquire() as connection:
            async with connection.transaction():
                ...
                query1 = """SELECT status FROM users WHERE did = $1"""

                status = await connection.fetchval(query1, did)

                if not handle and not pds:
                    if status is True:
                        query2 = """UPDATE users SET status = FALSE, reason = $2 WHERE did = $1"""

                        await connection.execute(query2, did, "unresponsive")

                        query3 = """INSERT INTO did_web_history (did, status, timestamp) VALUES ($1, $2, $3)"""

                        await connection.execute(query3, did, False, datetime.now())

                        return
                elif handle or pds:
                    if status is False:
                        query4 = """UPDATE users SET status = TRUE WHERE did = $1"""

                        await connection.execute(query4, did)

                        query5 = """INSERT INTO did_web_history (did, status, timestamp) VALUES ($1, $2, $3)"""

                        await connection.execute(query5, did, True, datetime.now())
                ...
                if user_info:
                    ...
                    if update_pds and update_handle:
                        await connection.execute("""UPDATE users SET handle = $2, pds = $3 WHERE did = $1""", did,
                                                 handle, pds)
                    elif update_pds and not update_handle:
                        await connection.execute("""UPDATE users SET pds = $2 WHERE did = $1""", did, pds)
                    elif update_handle and not update_pds:
        ...
        logger.error(f"Error updating did:webs {e}")
...
read_dbs = [db for db in database_config if read_keyword and "database" in db.lower()]
